...
async def delete_muscle(message: Message):
  if str(message.from_user.id) in ADMINS:
    ...
    await DatabaseManager(model=Training, session_maker=SessionLocal).delete(int(id_))
...
async def get_table(message: Message):
  if str(message.from_user.id) in ADMINS:
    ...
    if table_name in TABLES:
      model = Base.metadata.tables[table_name]  # Предполагается, что table_name соответствует имени таблицы в базе данных
      ...
      await message.answer_document(input_file, caption=f'Структура таблицы   {table_name}')
